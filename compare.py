...
# 32 uses an inconsistent url, no space like the others.  Dumb mistake on someone's part
models_dict = { '2': '(2 vcpu, 4.5 gb),(2 vcpu, 5.5 gb),(2 vcpu, 6.5 gb)',
                '2_4': '(2 vcpu, 4.5 gb)', 
                '2_5': '(2 vcpu, 5.5 gb)', 
                '2_6': '(2 vcpu, 6.5 gb)', 
                '4': '(4 vcpu, 9 gb)', 
...
def argue_with_me() -> None:
    ...
    parser.add_argument('-m', '--m_type', type=str, help='Model Type (pa or vm)', required=True)
    ...
    if m_type == 'vm' and all: 
        m_number = list(set(models_dict.keys()))[1:]  # all VMs except the 0 index key, since it covers all of the 2vCPU, so it would be redundant
...
def create_end_url(m_type: str, m_number: list) -> str:
    ...
    if m_type == 'vm':
        ...
        print(f"Improper model type ->{m_type}<- (pa or vm only).  Exiting...")
        sys.exit()
    return url
...
def dialog_get_models() -> tuple:
    ...
    """
    m_number = []
    m_type = input("Enter model type (pa/vm) [pa]: ")
    m_type = m_type.lower()
    if m_type == '':
        m_type = 'pa'
    print("[q or enter to exit]")
    if m_type == 'vm':
        for i,v in enumerate(models_dict.keys(), 1):
                print(f"{i}.  {v}")
    while True:
